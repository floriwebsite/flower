...
<head>
 ...
 $(function(){
  ...
  $('#get_code').click(function(){   //发h送手机号码获取验证码
  ...
  return false;
    ...
    // 接收后端的验证，显示相应提示
         ...
         $.post("<?php echo U('Index/artistRegister');?>",$(this).serialize(),function(data){

        if(data.flag==0){

        alert("请勿在获取验证码后变更您的手机号");//跳转到主页
...
<body style="background-color:#F2F2F2">
   ...
   <div class="keypoint  bg-inverse radius text-center" style="background-color:#B0E2FF;height:189px;">
      ...
      <br/>
      <br/>
      <h2 style="font-size:23px;">欢迎注册</h2>
     </div>
     <br>
     <br>
...
<div >
  ...
  <div class="form-group">
    <div class="label"><label for="username">*名字</label></div>
    <div class="field">
      <input type="text" class="input radius-rounded" id="name" name="username" size="50" data-validate="required:必填" placeholder="请输入您的名字" />
    </div>
  </div>
  ...
  <div class="form-group">
   <div class="label"><label class="label">*地区：</label></div>
   <select class="input radius-rounded" name="city">
      <option>成都</option>
      <option>重庆</option>
      <option>北京</option>
      <option>上海</option>
  ...
  <div class="form-group" style="display:none;" id="code_input">
    ...
    <div class="field">
      <input type="text" style="width:50%" class="input radius-rounded" id="code" name="code" size="50" data-validate="required:必填,plusinteger:请正确输入验证码" placeholder="输入验证码" />
    </div>
  </div>
